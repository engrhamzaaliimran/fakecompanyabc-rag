...
- comparison of retrieval-pipeline effects and judge disagreement
- manual validation on 5 questions, where I compared both judges against my own faithfulness and factual-correctness judgments
- data-driven entity-schema discovery for the GraphRAG stage
- GLiNER-based domain entity extraction
- an interactive HTML application for visual entity validation and confidence-threshold analysis

---

## GraphRAG Progress

The **GraphRAG stage is now in progress** using the `08` source-file series. I am deliberately validating each graph-construction stage before moving on to relationship extraction and Neo4j.

### 08.1 - Entity Schema Discovery

Rather than defining the knowledge-graph entity schema entirely by hand, I first used the policy corpus itself to discover useful entity categories.
...
HR Policies
    ...
    -> 25 Reviewed Entity Types
```

The clustering stage is used for **schema discovery**, not for final entity extraction. The discovered clusters and candidate concepts were manually reviewed before selecting the final 25 entity types supplied to GLiNER.

### 08.2 - GLiNER Entity Extraction

The reviewed schema is then passed to GLiNER together with the original policy chunks:

```text
25 Reviewed Entity Types
          +
Original Policy Chunks
          |
          v
       GLiNER
          |
          v
Text Span + Entity Type + Confidence + Character Offsets
```
...
The current extraction deliberately uses a **0.20 confidence floor**. The purpose of this lower threshold is to retain lower-confidence candidates for visual validation instead of permanently discarding them before inspection.
...
| Reviewed entity types | 25 |
| GLiNER detections | 361 |
| Unique entity/type pairs | 198 |
| Extraction confidence floor | 0.20 |

The `0.20` value is an **extraction and validation floor**, not a claim that every detection above 0.20 should enter the final knowledge graph. The final operating threshold can be selected after inspecting how entity quality changes as confidence increases.

### 08.2.1 - Interactive Entity Validation

I added a separate visualization stage to inspect GLiNER predictions directly against the source text.

The generated report contains the **entire 20-chunk corpus** and highlights the stored GLiNER detections in their original context. For every detected entity, the report shows:
...
The application also provides interactive filtering by:

- source document
- entity type
- minimum confidence threshold

This makes it possible to inspect **coverage, span boundaries, entity-type assignment, and confidence** before relation extraction.
...
```text
08.1 Schema Discovery
        |
        v
25 Reviewed Entity Types
        |
        v
08.2 GLiNER Extraction
        |
        v
361 Candidate Entity Mentions (confidence >= 0.20)
        |
        v
08.2.1 Interactive Visual Validation
        |
        v
Coverage / Span / Type / Confidence Inspection
        |
        v
...
├── selected_entity_schema.csv
├── gliner_entities.json
└── gliner_entity_validation.html
```

The current focus is **entity-extraction validation and confidence-threshold analysis**. Relationship extraction comes after this stage is understood and validated.
...
| `07_evaluate_rag_ragas.py` | Run the 100-question dense-RAG generation baseline and evaluate it with a local RAGAS/Qwen judge. |
| `07_evaluate_rag_tfidf_ragas.py` | Run the same 100-question generation benchmark with TF-IDF policy routing before dense retrieval. |
| `08.1_entity_creation.py` | Discover candidate graph entity types from the policy corpus using spaCy candidates, Nomic embeddings, PCA, HDBSCAN clustering, and manual review. |
| `08.2_entity_extraction_gliner.py` | Extract domain-specific entity mentions with GLiNER using the 25 reviewed entity types from 08.1. |
| `08.2.1_visualize_gliner_entities.py` | Generate the self-contained interactive HTML report used to visually validate entity coverage, span boundaries, types, and confidence scores. |
...
## Current GraphRAG Stage - Entity Validation Before Neo4j

GraphRAG is being added to this same repository rather than developed as a separate project. This keeps the dataset and, where appropriate, the benchmark questions shared across retrieval approaches.
...
The current graph-building progression is:

```text
08.1  Entity-schema discovery
  -> 25 manually reviewed entity types

08.2  GLiNER entity extraction
  -> candidate entity spans + types + confidence + offsets

08.2.1 Interactive entity validation
  -> inspect the complete chunked corpus
  -> analyze confidence thresholds
  -> identify missed, weak, or incorrectly classified entities
...
Next:
  ...
  -> Neo4j graph construction
  -> GraphRAG retrieval
  -> comparison with the classical RAG baselines
```

The goal is not simply to generate a graph automatically. Each intermediate representation is inspected before it is promoted into the final knowledge graph.
